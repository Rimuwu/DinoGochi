...
async def save_kd(dino_id: ObjectId, activity_type: str, kd_time: int) -> dict:
    ...
    if not await kd_activity.find_one({'dino_id': dino_id, 'activity_type': activity_type}):
        kd = {
            'dino_id': dino_id,
...
async def check_activity(dino_id: ObjectId, activity_type: str) -> int:
    ...
    fr = await kd_activity.find_one({'dino_id': dino_id, 'activity_type': activity_type})
    if not fr: return 0
    else:
        time_ost = fr['kd_time'] - int(time_now())
        if time_ost < 0:
            await kd_activity.delete_one({'_id': fr['_id']})
            return 0
...
async def check_all_activity(dino_id: ObjectId):
    ...
    """
    fr_l = await kd_activity.find({'dino_id': dino_id})
    result_dict = {}

    for i in fr_l:
        time_ost = i['kd_time'] - int(time_now())
        if time_ost < 0:
            await kd_activity.delete_one({'_id': i['_id']})
        else:
